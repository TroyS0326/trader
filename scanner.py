...
def get_trade_decision(model_scores: Dict[str, int], time_et: datetime, relative_strength_vs_spy: float) -> str:
    """Legacy time-bucketed decision. Active path uses regime_trade_decision() from decision.py."""
    ...
    if 10 * 60 + 30 < minutes < 11 * 60:
        logger.debug("get_trade_decision: %s in late-morning dead zone (10:31-10:59 ET)", time_et.strftime('%H:%M'))
        return 'WATCH FOR BREAKOUT'
    ...
    if 14 * 60 < minutes < 15 * 60:
        logger.debug("get_trade_decision: %s in pre-power-hour dead zone (14:01-14:59 ET)", time_et.strftime('%H:%M'))
        return 'WATCH FOR BREAKOUT'
    if 15 * 60 <= minutes <= 16 * 60:
        if relative_strength_vs_spy > 2.0 and model_scores['tradability'] > 55:
            return 'BUY NOW'
        return 'WATCH FOR BREAKOUT'
    logger.debug("get_trade_decision: %s outside all active windows", time_et.strftime('%H:%M'))
    return 'WATCH FOR BREAKOUT'
