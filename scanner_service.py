...
_CIRCUIT_FAIL_TTL   = 300
_CIRCUIT_OPEN_TTL   = 300
...
def _record_dispatch_failure(user_id, symbol, exc) -> None:
    try:
        count = redis_client.incr(_CIRCUIT_FAIL_KEY)
        redis_client.expire(_CIRCUIT_FAIL_KEY, _CIRCUIT_FAIL_TTL)
        logger.error("DISPATCH_FAILURE user_id=%s symbol=%s error=%s consecutive_failures=%s limit=%s", user_id, symbol, exc, count, _CIRCUIT_FAIL_LIMIT)
        if int(count) >= _CIRCUIT_FAIL_LIMIT:
            redis_client.setex(_CIRCUIT_OPEN_KEY, _CIRCUIT_OPEN_TTL, "1")
            logger.critical("CIRCUIT_BREAKER_TRIPPED consecutive_failures=%s — all scan dispatch halted for %ss", count, _CIRCUIT_OPEN_TTL)
    except Exception as redis_exc:
        logger.warning("_record_dispatch_failure Redis write failed: %s", redis_exc)


def _reset_dispatch_circuit_breaker(user_id, symbol) -> None:
    try:
        existing = redis_client.get(_CIRCUIT_FAIL_KEY)
        if existing and int(existing) > 0:
            redis_client.delete(_CIRCUIT_FAIL_KEY)
            logger.info("CIRCUIT_BREAKER_RESET user_id=%s symbol=%s — failure counter cleared", user_id, symbol)
    except Exception as exc:
...
def _dispatch_execution_if_allowed(user: Any, scan_payload: Dict[str, Any]) -> None:
    ...
    if _circuit_breaker_is_open():
        logger.critical("CIRCUIT_BREAKER_OPEN — dispatch suppressed user_id=%s symbol=%s", getattr(user, "id", None), str(diag.get("symbol") or "").strip().upper())
        return
    ...
    try:
        ...
        _reset_dispatch_circuit_breaker(getattr(user, "id", None), str(diag.get("symbol") or "").strip().upper())
    except Exception as dispatch_exc:
        _record_dispatch_failure(getattr(user, "id", None), str(diag.get("symbol") or "").strip().upper(), dispatch_exc)
        return
